...
export default function MealIdeas({ ingredient }) {
    ...
    return (
        <div>
            <h2 className="text-3xl font-bold m-2 text-gray-800">Meal Ideas</h2>
            {ingredient === "" ? (
                <h2 className="text-xl font-bold m-2 text-gray-500">Select an ingredient to see meal ideas</h2>
            ) : (
                <h2 className="text-xl font-bold m-2 text-gray-700">Here are some meal ideas using {ingredient}:</h2>
            )}
            {meals !== null ? (
                <>
                    {meals.map((meal, index) => (
                        <div
                            key={index}
                            className={`p-2 m-4 bg-blue-100 max-w-sm hover:outline-none hover:ring-2 hover:ring-blue-500 hover:ring-opacity-75 flex ${
                                meal.idMeal === id ? "scale-110" : ""
                            } duration-300 ease-out`}
                            onClick={() => setId(meal.idMeal)}
                        >
                            <div className={`flex align-middle justify-center ${meal.idMeal === id ? "flex-col" : "flex-row"} text-center w-full`}>
                                ...
                                <div className="flex justify-center align-middle flex-col">
                                    <h2 className="text-2xl font-semibold text-center align-middle justify-center m-2 text-gray-800">{meal.strMeal}</h2>
                                    {meal.idMeal === id ? (
                                        <p className="overflow-scroll text-sm h-full text-blue-600">
            ...
            {meals === null && ingredient !== "" ? (
                <h2 className="text-xl font-bold m-2 text-red-500">No meal ideas found for {ingredient}</h2>
            ) : (
                ""
            )}
        </div>
